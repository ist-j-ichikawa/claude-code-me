...
const api = new Hono()
  ...
  .get("/env", (c) => {
    const settings = readJsonFile(join(HOME_CLAUDE_DIR, "settings.json")) as
  ...
  .get("/history", zValidator("query", historyQuery), (c) => {
    ...
    return c.json(getHistory(HOME_CLAUDE_DIR, limit));
  })
  .get("/insights", (c) => {
    const reportPath = join(HOME_CLAUDE_DIR, "usage-data", "report.html");
    try {
